...
//description
 ...
 //exicute
    if(empty($location_err) &&  empty($date_err) && empty($time_err) && empty($price_err) && empty($description_err)){
        ...
        $sql = "UPDATE Events SET location = ?, time = ?, name = ?, price = ?, description = ?, date = ? WHERE EventID = '$event_id'";

        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "ssssss", $param_location,  $param_time, $param_name, $param_price, $param_description, $param_date);
