...
$location = $date = $time = $price = $description = "";
$location_err = $date_err = $time_err = $price_err = $description_err = "";
...
$sql = "SELECT user_id FROM LoginOrganization WHERE username = '$session_user'";
 ...
 //exicute
    if(empty($location_err) &&  empty($date_err) && empty($time_err) && empty($price_err) && empty($description_err)){
        ...
        $sql = "INSERT INTO Events (location, time, name, price, description, date, OrganizationID) VALUES (?, ?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssssss", $param_location,  $param_time, $param_name, $param_price, $param_description, $param_date, $param_org_user_id);
