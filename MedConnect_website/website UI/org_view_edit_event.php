    ...
    <main>
        ...
        <div class="admin_patient">
            <div class="table">
                ...
                $sql = "SELECT EventID, name, location, date, time, price, description, attendance
                FROM Events WHERE OrganizationID = '$org_user_id'";
                if($result = mysqli_query($link, $sql)){
                    if(mysqli_num_rows($result) > 0){
                		echo '<form action="org_view_exicute_event.php" method="POST"><table><tr><th>Edit this event?</th><th>Name</th><th>Location</th><th>Date</th><th>Time</th><th>Price</th><th>description</th><th>attendance</th></tr>';
                        while($row = mysqli_fetch_array($result)){
                			echo "<tr><td>"."<input type='radio' name='edit' value=".$row['EventID']." required>"."</td><td>".$row['name'] ."</td><td>". $row['location']."</td><td>". $row['date']."</td><td>". $row['time']."</td><td>". $row['price']."</td><td>". $row['description']."</td><td>". $row['attendance']."</td></tr>"; }
                		echo "</table><input class='button' type='submit' value='Edit'></form>";
                	}
                	else{
                		echo "no result";
                	}
